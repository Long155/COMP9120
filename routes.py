...
def list_admission():
    ...
    # Otherwise try to get from the database
        ...
        if (search_term.strip() == ''): # If the search term is empty
            admission_list_find = database.findAdmissionsByAdmin(user_details['login'])
        else:
            print("Searching for keyword:", search_term)    
...
def update_admission():
    ...
    if (request.method == 'GET'):
        discharge_date = request.args.get('discharge_date')
        print("discharge_date:",discharge_date)
        if discharge_date is not None:
            datelen = len(discharge_date)
        else:
            datelen = 0
        if (datelen > 0):
            discharge_date = datetime.strptime(request.args.get('discharge_date'), '%Y-%m-%d').date() #修改了日期格式
        else:
            discharge_date = ''
    ...
    success = database.updateAdmission(request.form['admission_id'], #疑问这里能修改吗
                                        request.form['type'],
                                        request.form['department'],
                                        discharge_date,
                                        request.form['fee'],
...
def check_login(login, password):
    ...
    if userInfo is None:
        print("userInfo is None! No user found")
        return None
    else:
        tuples = {
